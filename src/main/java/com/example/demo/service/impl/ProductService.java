...
//        product.setIdProducer(producerRepo.findByProducerName(productRequestDto.getProductName()));
    ...
    public Page<ProductUserResponseDto> getHomePageProduct(Pageable page){
        return producerRepo.findProductHomePageByProducts(page);
    }

    @Override
    public List<Top10SaleResponseDto> findByTop10Sell(){
        return productRepo.findTop10BestSellingProducts();
    }

    @Override
    public Integer getToatalStockInstore(LocalDateTime starDate, LocalDateTime endDate){
        return productRepo.getTotalStockQuantityInDateRange(starDate,endDate);
    }
